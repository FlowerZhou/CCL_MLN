...
class SocialModelling(object):
    """
    处理predicate、formula、evidence文件
    """
    ...
    def read_formula(self, paths, flag):
        """
        处理formula文件
        flag: if formulas have weight, True; else, False
        """
        formula = []
        base_path = os.getcwd()
        ...
        if flag:
            ...
            formula = ['0.5' + x for x in formula if 'Exists ' not in x]
    ...
    def model_config(self, predicate, formula, database, mln_path, db_path):
        """
        生成MLN实例, 输入predicates, formulas, evidences
        """
        base_path = os.getcwd()
...
def test_a():
    """
    dataset: Simple Smoke dataset, very fast
    """
    sm = SocialModelling()

    predicate = sm.read_predicate('../predicate.txt')
    formula = sm.read_formula('../formula1.txt', True)
...
def test_b():
    """
    dataset: UW dataset, medium dataset
    """
    sm = SocialModelling()
    predicate = sm.read_predicate('../pred.txt')
    formula = sm.read_formula('../formula2.txt', False)
    database = sm.read_data('../data1.txt')
    # pdb.set_trace()
    data, mln = sm.model_config(predicate, formula, database, 'school.mln', 'school.db')
    # mln = MarkovLogicNet(grammar='StandardGrammar', logic='FirstOrderLogic')
    # mln_ = mln.load('uw.mln')
    # db = DataBase(mln_)
    # data = db.load('language.db')
    # pdb.set_trace()
    output = sm.activate_model(data, mln)
    output.to_file(os.getcwd() + '/' + 'learnt_school_mln.mln')
    # pdb.set_trace()
    sm.inference_str('advisedBy(Person18,Person248)', output, data, mln)
...
def test_c():
    """
    dataset: bigger UW dataset, has more constants
    """
    sm = SocialModelling()
    predicate = sm.read_predicate('../pred.txt')
    formula = sm.read_formula('../formula2.txt', False)
    database = sm.read_data('../data3.txt')
    # pdb.set_trace()
    data, mln = sm.model_config(predicate, formula, database, 'school.mln', 'school.db')
    # mln = MarkovLogicNet(grammar='StandardGrammar', logic='FirstOrderLogic')
    # mln_ = mln.load('uw.mln')
    ...
    output.to_file(os.getcwd() + '/' + 'learnt_school_mln.mln')
    # pdb.set_trace()
    sm.inference_str('advisedBy(Person70,Person240)', output, data, mln)


@pytest.mark.IE
def test_d():
    """
    dataset: ie dataset, bigger than UW_B, very slow
    """
    sm = SocialModelling()
    predicate = sm.read_predicate('../predicate_ie.txt')
    formula = sm.read_formula('../formula_ie.txt', True)
    database = sm.read_data('../data_ie.txt')
    # pdb.set_trace()
    data, mln = sm.model_config(predicate, formula, database, 'ie.mln', 'ie.db')
    # mln = MarkovLogicNet(grammar='StandardGrammar', logic='FirstOrderLogic')
    # mln_ = mln.load('uw.mln')
    # db = DataBase(mln_)
    # data = db.load('language.db')
    # pdb.set_trace()
    output = sm.activate_model(data, mln)
    output.to_file(os.getcwd() + '/' + 'learnt_ie_mln.mln')
    # pdb.set_trace()
    sm.inference_str('Next(P00, P22)', output, data, mln)
