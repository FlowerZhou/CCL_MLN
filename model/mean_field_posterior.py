...
class FactorizedPosterior(nn.Module):
  def __init__(self, graph, latent_dim, slice_dim=5, mln=None):
    ...
    if cmd_args.load_method == 1:
      ...
      for idx in range(self.num_rels):
        ...
        num_args = self.mln._predicates[rel].num_args
  ...
  def forward(self, latent_vars, node_embeds, batch_mode=False, fast_mode=False, fast_inference_mode=False):
    ...
    :param latent_vars: list of latent variables (i.e. unobserved facts)
    :param node_embeds: node embeddings
    :return: n-dim vector, probability of corresponding latent variable being True
    """
